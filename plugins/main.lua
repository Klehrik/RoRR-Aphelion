...
gui.add_imgui(function()
    if ImGui.Begin("Aphelion Debug") then
        ...
        elseif ImGui.Button("Create custom object") then
            ...
            local player = Player.get_client()
            local inst = gm.instance_create_depth(player.x + (player.image_xscale * 24), player.y, 0, gm.constants.oHuntressBolt1)
            inst.parent = player
            inst.direction = 90 - (player.image_xscale * 90)
            inst.hspeed = inst.hspeed * player.image_xscale
            inst.damage_coeff = 100
